...
axiosApiInstance.interceptors.response.use(
  (response) => {
    ...
    let currentPage = 1
    if( response.config && response.config.url && response.config.url.includes('page')) {
      const page = parseInt(response?.config.url.split('page=')[1])
      currentPage = page
    }
   
    // Agrega la información de paginación a los datos de la respuesta
